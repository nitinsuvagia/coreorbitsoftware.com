...
async function handleSubscriptionDeleted(subscription: Stripe.Subscription): Promise<void> {
  ...
  if (sub) {
    ...
    await eventBus.publishToTopic('billing-subscription-canceled' as any, 'billing.subscription_canceled', {
      subscriptionId: sub.id,
      tenantId: sub.tenantId,
...
async function handleCheckoutSessionCompleted(session: Stripe.Checkout.Session): Promise<void> {
  ...
  // Publish to topic for notification service
  const eventBus = getEventBus('billing-service');
  await eventBus.publishToTopic('billing-subscription-activated' as any, 'billing.subscription_activated', {
    tenantId,
    planName: plan.name,
    planId,
    billingCycle,
...
async function handleInvoicePaymentFailed(invoice: Stripe.Invoice): Promise<void> {
  ...
  if (subscriptionId) {
    ...
    if (subscription) {
      ...
      // Publish to topic for notification service
      const eventBus = getEventBus('billing-service');
      await eventBus.publishToTopic('billing-payment-failed' as any, 'billing.payment_failed', {
        subscriptionId: subscription.id,
        tenantId: subscription.tenantId,
        stripeInvoiceId: invoice.id,
